  ...
  try {
    ...
    const stats = {
      ...
      averageProgress: skills.length > 0 
        ? Math.round(skills.reduce((sum: number, skill: any) => sum + skill.current_level, 0) / skills.length)
        : 0,
      skillsImproving: skills.filter((skill: any) => skill.current_level > 0 && skill.current_level < skill.target_level).length,
      skillsToFocus: skills.filter((skill: any) => skill.current_level < 50).length,
      skillsCompleted: skills.filter((skill: any) => skill.current_level >= skill.target_level).length
    }
