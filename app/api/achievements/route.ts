...
export async function GET(request: NextRequest) {
  try {
    ...
    const completedIds = completedAchievements.map((a: any) => a.achievement_id)
    ...
    const achievementData = {
      completed: completedAchievements.map((ach: any) => ({
      ...
      stats: {
        ...
        pointsEarned: completedAchievements.reduce((total: number, ach: any) => total + getPointsForRarity(ach.rarity), 0)
      }
    }
