...
class EfficientTranslationService {
  ...
  async translateContent(targetLang: SupportedLanguage): Promise<{
    ...
    // For Telugu and Marathi, prefer API but fallback to offline
    if (targetLang === 'te' || targetLang === 'mr') {
      ...
      try {
        ...
        try {
          if (targetLang === 'te') {
            ...
            const { marathiTranslations } = await import('./offline-translations')
            this.translationCache.set(targetLang, marathiTranslations)
            return {
