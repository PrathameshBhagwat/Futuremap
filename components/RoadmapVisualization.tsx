...
export default function RoadmapVisualization({ roadmap }: RoadmapVisualizationProps) {
  ...
  let extractedPhases = roadmap.phases || roadmap.roadmap_data?.phases || []
  
  // Ensure phases is always an array
  if (!Array.isArray(extractedPhases)) {
    console.warn('⚠️ Phases is not an array, attempting to convert...')
    // If phases is an object, convert it to an array
    if (typeof extractedPhases === 'object') {
      extractedPhases = Object.keys(extractedPhases).map((key: any, index: number) => {
        const phaseData = extractedPhases[key]
        return {
