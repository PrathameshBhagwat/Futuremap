...
function RoadmapScene3D({ roadmap }: { roadmap: Enhanced3DRoadmapProps['roadmap'] }) {
  ...
  const nodes3D = useMemo(() => {
    ...
    // Extract and normalize phases
    let phasesData = roadmap.roadmap_data?.phases
    ...
    if (!Array.isArray(phasesData)) {
      ...
      phasesData = Object.keys(phasesData).map((key: any, index: number) => {
        const phaseData = phasesData[key]
        return {
    ...
    console.log('🎮 Generating 3D nodes from roadmap phases:', phasesData.length)
