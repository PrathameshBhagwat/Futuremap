...
export async function POST(request: NextRequest) {
  try {
    ...
    // Normalize phases - ensure it's always an array
    ...
    if (!Array.isArray(phases)) {
      ...
      phases = Object.keys(phases).map((key: any, index: number) => {
        const phaseData = phases[key]
        return {
          id: phaseData.id || `phase-${index}`,
          title: phaseData.title || key,
    ...
    // Update the roadmapData with normalized phases
    roadmapData.phases = phases
    ...
    // Include the additional AI recommendations and timeline
    // These are now stored in roadmap_data
    const enrichedRoadmap = {
      ...savedRoadmap,
      ai_recommendations: savedRoadmap.roadmap_data?.ai_recommendations || roadmapData.recommendations || {},
      timeline: savedRoadmap.roadmap_data?.timeline || roadmapData.timeline || {}
    }
