    ...
    try {
      ...
      const stats = {
        ...
        roadmapProgress: userSkills.length > 0 
          ? Math.round(userSkills.reduce((sum: number, skill: any) => sum + (skill.current_level || 0), 0) / userSkills.length)
          : 0,
        ...
        highestSkillLevel: userSkills.length > 0 
          ? Math.max(...userSkills.map((skill: any) => skill.current_level || 0))
          : 0,

        skillCategories: [...new Set(userSkills.map((skill: any) => skill.category).filter(Boolean))],
        ...
        recentActivityCount: activities.filter((activity: any) => 
        ...
        achievementTypes: [...new Set(achievements.map((a: any) => a.type).filter(Boolean))],
        rareAchievements: achievements.filter((a: any) => a.rarity === 'epic' || a.rarity === 'legendary').length,
        ...
        // Goal tracking
        skillsNearTarget: userSkills.filter((skill: any) => 
          skill.target_level && skill.current_level >= skill.target_level * 0.8
        ).length,
        
        skillsNeedingAttention: userSkills.filter((skill: any) => 
      ...
      const detailedStats = {
        ...stats,
        recentQuizzes: quizResults.slice(0, 5).map((quiz: any) => ({
          id: quiz.id,
...
function getTopCareerPaths(quizResults: any[]): string[] {
  const careerCounts = quizResults.reduce((acc: any, quiz: any) => {
    if (quiz.career_path) {
...
function determineLearningTime(activities: any[]): string {
  ...
  const hours = activities.map((activity: any) => new Date(activity.created_at).getHours())
  const avgHour = hours.reduce((sum: number, hour: number) => sum + hour, 0) / Math.max(hours.length, 1)
...
function analyzeSavedColleges(savedColleges: any[]): any {
  ...
  const locations = savedColleges.map((college: any) => college.location || college.state).filter(Boolean)
  const types = savedColleges.map((college: any) => college.type).filter(Boolean)
  
  return {
    preferredLocations: [...new Set(locations)].slice(0, 5),
    preferredTypes: [...new Set(types)],
    averageRating: savedColleges.reduce((sum: number, college: any) => sum + (college.rating || 0), 0) / savedColleges.length
  }
}
