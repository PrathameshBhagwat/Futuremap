  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (typeof data.response === 'string') {
        ...
        // If response is object, try to extract readable content
        if (data.response.message) {
          assistantContent = data.response.message
          if (data.response.steps && Array.isArray(data.response.steps)) {
            assistantContent += '\n\n' + data.response.steps.join('\n')
          }
        } else {
      ...
      if (error instanceof Error) {
        if (error.message.includes('timeout')) {
          errorContent = '⏱️ Request timed out. The AI service is taking too long to respond. Please try again in a moment.'
        } else if (error.message.includes('quota') || error.message.includes('rate limit')) {
          errorContent = '⚡ API quota reached. The free AI service is experiencing high demand. Please try again in a few minutes.'
          ...
          errorContent = '⚠️ The AI service is temporarily unavailable. Please try again shortly.'
