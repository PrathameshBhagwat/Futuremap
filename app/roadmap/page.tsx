...
function RoadmapOverview({ roadmap, onStartJourney, savedRoadmapId }: {
  ...
  const phasesArray = (Array.isArray(roadmap.phases) 
    ? roadmap.phases 
    : Object.values(roadmap.phases || {})) as any[]
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <DownloadRoadmapButton
          roadmapId={savedRoadmapId || roadmap.id || 'latest'}
          careerGoal={roadmap.title?.replace(' Learning Path', '') || 'Career'}
          variant="secondary"
        />
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        >
          ...
          {phasesArray.map((phase: any, index: number) => (
            <div key={phase.id} className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700">
              ...
              <div className="grid grid-cols-2 gap-2">
                {phase.nodes && phase.nodes.slice(0, 4).map((node: any) => (
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-neon-cyan">
                  {phasesArray.reduce((sum: number, phase: any) => sum + (phase.nodes?.length || 0), 0)}
                </div>
                <div className="text-sm text-gray-400">Learning Modules</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-neon-pink">
                  {phasesArray.reduce((sum: number, phase: any) => 
                    sum + (phase.nodes?.filter((node: any) => node.type === 'project').length || 0), 0
                  )}
                </div>
...
export default function RoadmapPage() {
  ...
  const [visualizationMode, setVisualizationMode] = useState<'network' | 'timeline'>('network')
  ...
  const handleStartJourney = async () => {
    ...
    try {
      ...
      const phasesArray = (Array.isArray(generatedRoadmap.phases)
        ...
        : Object.values(generatedRoadmap.phases || {})) as any[]
      ...
      const response = await fetch('/api/roadmap', {
        ...
        body: JSON.stringify({
          ...
          phases: phasesArray,
          nodes: phasesArray.flatMap((phase: any) => phase.nodes || []),
          connections: phasesArray.map((phase: any, index: number) => ({
  ...
  return (
    <div className="relative min-h-screen flex flex-col" style={{ background: '#0B0E17' }}>
      {/* Header - Sticky */}
      <div className="sticky top-16 z-40 bg-gradient-to-b from-black via-black to-black/80 pt-8 pb-8 border-b border-gray-800/50 backdrop-blur-sm">
          ...
          >
            ...
            {(currentView === '3d-view' || currentView === 'interactive') && (
              <div className="flex space-x-2">
                <div className="flex space-x-1 bg-gray-800 rounded-lg border border-gray-700 p-1">
                  <button
      ...
      <div className="flex-1 relative z-20 pb-32 overflow-y-auto">
        <AnimatePresence mode="wait">
          ...
          {currentView === 'interactive' && savedRoadmap && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.5 }}
              className="flex flex-col h-[calc(100vh-200px)] px-4 space-y-4 pointer-events-auto"
            >
              {/* View Mode Selector */}
              <div className="flex items-center gap-2 bg-gray-800/50 border border-gray-700 rounded-lg p-2 w-fit z-30 relative pointer-events-auto">
                <button
                  onClick={() => setVisualizationMode('network')}
                  className={`px-4 py-2 rounded transition-colors cursor-pointer relative z-30 ${
                    visualizationMode === 'network'
                      ? 'bg-neon-cyan text-black font-semibold'
                      : 'text-gray-300 hover:text-white'
                  }`}
              ...
              <div className="flex-1 bg-gray-900/50 rounded-xl border border-gray-800 overflow-hidden min-h-0">
                {visualizationMode === 'network' ? (
                  ...
                  <TimelineVisualization
                    roadmap={{
                      ...
                      title: savedRoadmap.title,
                      description: savedRoadmap.description || '',
                      phases: savedRoadmap.phases || generatedRoadmap?.phases || [],
                      totalDuration: savedRoadmap.totalDuration || generatedRoadmap?.totalDuration || 0
                    }}
                  />
                )}
              </div>
          ...
          {currentView === '3d-view' && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.5 }}
              className="h-[calc(100vh-200px)] px-4 rounded-2xl border border-gray-700 overflow-hidden bg-gray-900/50 backdrop-blur-sm"
            >
              <WebGLErrorBoundary>
