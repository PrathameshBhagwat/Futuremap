...
export default function RoadmapDetailsPage() {
  ...
  const phasesRaw = roadmap.roadmap_data?.phases || []
  
  // Ensure phases is always an array
  let phasesNormalized: Phase[] = []
  if (Array.isArray(phasesRaw)) {
    phasesNormalized = phasesRaw
  } else if (typeof phasesRaw === 'object') {
    // Convert object to array if needed
    console.warn('⚠️ Phases is an object, converting to array...')
    phasesNormalized = Object.keys(phasesRaw).map((key: any, index: number) => {
      const phaseData = (phasesRaw as any)[key]
      return {
        id: (typeof phaseData === 'object' ? phaseData.id : null) || `phase-${index}`,
        title: (typeof phaseData === 'object' ? phaseData.title : null) || key,
        description: (typeof phaseData === 'object' ? phaseData.description : null) || '',
        duration: (typeof phaseData === 'object' ? phaseData.duration : null) || '2 months',
        milestones: (typeof phaseData === 'object' ? phaseData.milestones : null) || [],
        completed: (typeof phaseData === 'object' ? phaseData.completed : null) || false,
        progress: (typeof phaseData === 'object' ? phaseData.progress : null) || 0
      }
    })
  }
  
  const phases = phasesNormalized
  ...
  return (
    <div className="min-h-screen bg-space-dark text-white relative overflow-hidden pt-0">
      {/* Enhanced Background */}
      ...
      <div className="relative z-10">
        ...
        {/* Main Content */}
        <div className={`${isFullscreen ? 'fixed inset-0 z-50 bg-space-dark pt-32' : 'relative pt-8'}`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
...
function DetailedView({ 
  ...
  if (Array.isArray(phasesRaw)) {
    ...
    // Convert object to array if needed
    phases = Object.keys(phasesRaw).map((key: any, index: number) => {
      const phaseData = (phasesRaw as any)[key]
      return {
        id: (typeof phaseData === 'object' ? phaseData.id : null) || `phase-${index}`,
        title: (typeof phaseData === 'object' ? phaseData.title : null) || key,
