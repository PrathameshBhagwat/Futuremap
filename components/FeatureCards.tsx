...
// This will be generated dynamically in the component
...
const AdditionalFeatureCard = ({ feature, index }: { feature: any, index: number }) => {
  const ref = useRef(null)
  const isInView = useInView(ref, { once: true, amount: 0.1 })
  const IconComponent = feature.icon
  const router = useRouter()
  ...
  const translatedFeatures = [
    {
      id: '3d-career-tree',
      icon: Sparkles,
      title: translations?.featureCards?.careerVisualization?.title || '3D Career Visualization',
      description: translations?.featureCards?.careerVisualization?.description || 'Interactive 3D maps to explore career paths and opportunities',
      color: 'neon-cyan',
      gradient: 'from-neon-cyan to-blue-500'
    },
    {
      id: 'college-finder',
      icon: MapPin,
      title: translations?.featureCards?.collegeRecommendations?.title || 'Smart College Recommendations',
      description: translations?.featureCards?.collegeRecommendations?.description || 'Find the perfect colleges based on your preferences and career goals',
      color: 'neon-pink',
      gradient: 'from-neon-pink to-purple-500'
    },
    {
      id: 'quiz',
      icon: Clock,
      title: translations?.featureCards?.personalizedQuiz?.title || 'Personalized Career Quiz',
      description: translations?.featureCards?.personalizedQuiz?.description || 'AI-driven assessment to discover your strengths and ideal career paths',
      color: 'neon-purple',
      gradient: 'from-neon-purple to-indigo-500'
    }
  ]
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          >
            {translations?.featureCards?.sectionSubtitle || 'Discover the tools and insights you need to make informed decisions about your career and education path.'}
          </motion.p>
        </motion.div>
        ...
        >
          {[
            {
              icon: Zap,
              title: translations?.featureCards?.additionalFeatures?.aiInsights?.title || 'AI-Powered Insights',
              description: translations?.featureCards?.additionalFeatures?.aiInsights?.description || 'Get personalized recommendations based on your skills and interests',
              color: 'neon-cyan',
              href: '/roadmap'
            },
            {
              icon: BookOpen,
              title: translations?.featureCards?.additionalFeatures?.studyMaterials?.title || 'Study Materials',
              description: translations?.featureCards?.additionalFeatures?.studyMaterials?.description || 'Access curated resources for entrance exams and skill development',
              color: 'neon-pink',
              href: '/learning-resources'
            },
            {
              icon: Target,
              title: translations?.featureCards?.additionalFeatures?.goalSetting?.title || 'Goal Setting',
              description: translations?.featureCards?.additionalFeatures?.goalSetting?.description || 'Set and track academic and career goals with smart reminders',
              color: 'neon-purple',
              href: '/dashboard'
            }
          ].map((feature, index) => (
